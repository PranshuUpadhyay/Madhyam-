...
const data = res.data || {};
setDonors({
charities: data.charities?.length ? data.charities : defaultDonors.charities,
restaurants: data.restaurants?.length ? data.restaurants : defaultDonors.restaurants,
});
} catch (err) {
console.error('Error fetching donors, using defaults:', err.message);
...
<img src={divider} alt="divider" className="w-full max-w-xl" />
    ...
    <div className="bg-white shadow rounded p-4">
      {donors[tab].map((item, idx) => (
        <div key={idx} className="border-b py-2">
          <a href={item.link || "#"} className="font-semibold text-blue-600 hover:underline">
            {item.name}
          </a>
